# Baekya Protocol Relay Server Fly.io Deployment Script
...
Write-Host "Baekya Protocol Relay Server Fly.io Deployment Started" -ForegroundColor Green
...
# Get app name
$AppName = Read-Host "Enter Fly.io app name for relay server (will be created if doesn't exist)"
if (-not $AppName.trim()) {
...
Write-Host ""
Write-Host "Relay Server Configuration:" -ForegroundColor Cyan
...
$confirm = Read-Host "Proceed with relay server deployment? (y/N)"
...
try {
    ...
    fly secrets set RELAY_PASSWORD=$RelayPassword --app $AppName
    fly secrets set RELAY_LOCATION=$RelayLocation --app $AppName
    ...
    Write-Host "Password: $('*' * $RelayPassword.Length)" -ForegroundColor Cyan
    Write-Host "Status: $appUrl/api/relay-status" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "IMPORTANT: Copy the relay server URL for full node connection!" -ForegroundColor Yellow
    Write-Host "Full node connection URL: $appUrl" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Relay server Fly.io deployment completed successfully" -ForegroundColor Green
